...
def get_infohash_sha1_digest(decoded: dict) -> bytes:
    ...
    # Returns the digest (i.e., the series of raw bytes representing the hash).
...
def get_infohash_sha1_hexdigest(decoded: dict) -> str:
    info = bencode(decoded["info"])
    # returns the hexdigest (i.e., a UTF8 hexadecimal encoding of the raw bytes).
...
def get_torrent_file_listing(decoded: dict) -> dict[Dirname, dict | list[File]]:
    ...
    for (size, path_segments) in files:
        if (
            len(path_segments) == 1
        ):  # if the file isn't contained under any directories:
            ...
            # reset `node`, which keeps track of the deepest directory we've
            # travelled to so far
            node = root
            ...
            # After the loop has finished, `node` points to the final directory
