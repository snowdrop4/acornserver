...
class TestUpload(TestCase):
	def setUp(self) -> None:
		...
		# 
		self.artist_data = { 'artist-name': 'Test, Test, and Away!', }
	...
	def test_autofilled_contribution(self) -> None:
		...
		# Do the same for release_group, this time with two throwaways so we don't get overlapping PKs with artist.
		(_            , _                 ) = create_random_release_group()
		(_            , _                 ) = create_random_release_group()
		(release_group, release_group_data) = create_random_release_group()
		
		(contribution, contribution_data) = create_random_contribution(artist, release_group)
		
		data = \
			{ **{ 'model_pk-artist_pk': '2', }
			, **{ 'contribution_select-contribution': '1', } # The upload view deduces the release_group pk from the contribution pk
			, **self.release_data
			, **self.torrent_data }
