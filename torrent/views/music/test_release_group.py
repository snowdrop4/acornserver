...
class TestReleaseGroup(TestCase):
    ...
    def test_add(self) -> None:
        url = (
            reverse("torrent:music_release_group_add")
            + "?artist="
            + str(self.artist.pk)
        )
        ...
        request = cast(
            AuthedWSGIRequest,
            self.requestFactory.post(url, {**self.data, **self.contribution_data})
        )
        ...
        self.assertEquals(
            release_group.group_type, self.data["release_group-group_type"]
        )
    ...
    def test_edit(self) -> None:
        ...
        url = reverse(
            "torrent:music_release_group_edit", kwargs={"pk": release_group.pk}
        )
        ...
        self.assertEquals(
            release_group.group_type, modified_data["release_group-group_type"]
        )
