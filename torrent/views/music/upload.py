...
class PkProvider:
	...
	def __setitem__(self, pk_name, pk_value):
		try:
			...
			# TODO: Can't just return a `HttpResponseBadRequest` because we aren't actually in a view.
			# Only way to signal an error is use an exception here, but Http404 is technically wrong.
			# 
...
def upload(request):
	...
	all_forms = ('model_pk', 'contribution_select', 'release_select', 'artist', 'release_group', 'contribution', 'release', 'torrent')
	...
	if tuple(request.GET.keys()) not in valid_get_parameters.keys():
		valid_get_parameters_str = ', '.join('(' + ', '.join(k) + ')' for k in valid_get_parameters.keys())
		message = 'Invalid GET parameter(s). Valid GET parameters are: ' + valid_get_parameters_str
	...
	torrent_constructor = lambda *args, **kwargs: MusicTorrentForm(*args, request.FILES, **kwargs) if (request.method == 'POST') else MusicTorrentForm(*args, **kwargs)
