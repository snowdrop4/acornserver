...
def view(request: AuthedHttpRequest, pk: int) -> HttpResponse:
    ...
    releases_by_release_groups_by_roles: dict[
        str, dict[MusicReleaseGroup, QuerySet[MusicRelease]]
    ] = {}
...
def edit(request: AuthedHttpRequest, pk: int) -> HttpResponse:
    ...
    return render(
        request, "torrent/music/artist/edit.html", {"form": form, "artist": artist}
    )
...
class Delete(DeleteView):
    ...
    def post(self, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        if self.request.POST.get("confirm", "no") == "yes":
            # Only delete the artist if the current user has the permission to
            # delete artists, or if it was the current user created this artist.
            if user.has_perm("torrent.delete_musicartist") or user == artist.creator:
                ...
                messages.error(
                    self.request, "Insufficient permissions to delete artist."
                )
