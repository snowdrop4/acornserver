...
def add(request: AuthedHttpRequest) -> HttpResponse:
    try:
        get_params = fill_typed_get_parameters(
            request, {"release_group": (True, int, "must be an integer")}
        )
...
def view(request: AuthedHttpRequest, pk: int) -> HttpResponse:
    ...
    try:
        release = (
            MusicRelease.objects.select_related("release_group")
...
def edit(request: AuthedHttpRequest, pk: int) -> HttpResponse:
    ...
    return render(
        request, "torrent/music/release/edit.html", {"form": form, "release": release}
    )
...
class Delete(DeleteView):
    ...
    def post(self, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        if self.request.POST.get("confirm", "no") == "yes":
            # Only delete the release if the current user has the permission to
            # delete releases, or if it was the current user created this release.
            if user.has_perm("torrent.delete_musicrelease") or user == release.creator:
                try:
                    release.delete()
                    messages.deletion(self.request, "Deleted release.")
                    return redirect(
                        "torrent:music_release_group_view", pk=release.release_group.pk
                    )
                except models.ProtectedError:
                    messages.failure(
                        self.request,
                        "A release that contains torrents cannot be deleted.",
                    )
            else:
                messages.error(
                    self.request, "Insufficient permissions to delete release."
                )
