...
class MusicArtist(models.Model):
    ...
    artist_type = models.CharField(
        max_length=3, choices=ArtistType.choices, default=ArtistType.ARTIST
    )
    ...
    creator = models.ForeignKey(
        ...
        null=True,
        related_name="created_musicartists",
    )
...
class MusicReleaseGroup(models.Model):
    ...
    contributors = models.ManyToManyField(
        MusicArtist, through="MusicContribution", related_name="release_groups"
    )
    ...
    creator = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name="created_musicreleasegroups",
    )

    def get_earliest_release_date(self) -> datetime.date:
        return min(
            self.releases.values_list("date", flat=True), default=datetime.date(1, 1, 1)
        )
...
class MusicContribution(models.Model):
    ...
    artist = models.ForeignKey(
        MusicArtist, on_delete=models.CASCADE, related_name="contributions"
    )
    release_group = models.ForeignKey(
        MusicReleaseGroup, on_delete=models.CASCADE, related_name="contributions"
    )
    ...
    creator = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name="created_musiccontribution",
    )
...
class MusicRelease(models.Model):
    ...
    release_group = models.ForeignKey(
        MusicReleaseGroup, on_delete=models.PROTECT, related_name="releases"
    )
    ...
    creator = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name="created_musicreleases",
    )
    ...
    def __str__(self) -> str:
        return (
            ...
            f", label: '{self.label}'"
            f", cat#: '{self.catalog_number}'"
            f", format: '{self.release_format}'"
        )
...
class MusicTorrent(torrent.Torrent):
    ...
    uploader = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name="music_uploads",
    )
    downloaders = models.ManyToManyField(
        auth.get_user_model(),
        through="MusicTorrentDownload",
        related_name="downloaded_music_torrents",
    )
...
class MusicTorrentPeer(torrent.Peer):
    torrent = models.ForeignKey(
        MusicTorrent, on_delete=models.CASCADE, related_name="peers"
    )
...
class MusicTorrentDownload(models.Model):
    user = models.ForeignKey(
        auth.get_user_model(), on_delete=models.CASCADE, related_name="music_downloads"
    )
    torrent = models.ForeignKey(
        MusicTorrent, on_delete=models.CASCADE, related_name="downloads"
    )
