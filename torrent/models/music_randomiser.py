...
def create_random_contribution(
    artist: MusicArtist, release_group: MusicReleaseGroup
) -> tuple[MusicContribution, dict]:
    data = {
        "contribution-contribution_type": random.choice(
            MusicContribution.ContributionType.values
        )
    }

    args = {a.removeprefix("contribution-"): b for (a, b) in data.items()}
    contribution = MusicContribution.objects.create(
        **args, artist=artist, release_group=release_group
    )
