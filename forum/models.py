...
class ForumCategory(MPTTModel):
    ...
    latest_post_thread = models.ForeignKey(
        "ForumThread", on_delete=models.SET_NULL, null=True, blank=True
    )
...
class ForumThread(ForumContent):
    ...
    latest_post_author = models.ForeignKey(
        auth.get_user_model(), on_delete=models.SET_NULL, null=True, blank=True
    )
