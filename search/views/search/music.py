    ...
    if form.is_valid():
        ...
        if form.cleaned_data["artist_name"] and form.cleaned_data["release_group_name"]:
            ...
            query = MusicReleaseGroup.objects.filter(
                name__contains=form.cleaned_data["release_group_name"]
            ).filter(
                contributions__artist__name__contains=form.cleaned_data["artist_name"]
            )
        elif form.cleaned_data["artist_name"]:
            model_name = "artist"
            query = MusicArtist.objects.filter(
                name__contains=form.cleaned_data["artist_name"]
            )
