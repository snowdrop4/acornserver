...
def profile_view(request: AuthedHttpRequest, pk: int) -> HttpResponse:
    ...
    uploads = (
        user.music_uploads.select_related("release__release_group")
        .prefetch_related("downloads")
        .order_by("upload_datetime")[:10]
    )

    downloads = (
        user.music_downloads.select_related("torrent__release__release_group")
...
def profile_edit(request: AuthedHttpRequest) -> HttpResponse:
    ...
    return render(
        request, "account/user/edit_profile.html", {"form": form, "target_user": user}
    )
...
def account_edit(request: AuthedHttpRequest) -> HttpResponse:
    ...
    if (
        request.method == "POST"
        and "form_type" in request.POST
        and request.POST["form_type"] in forms
    ):
        form_type = request.POST["form_type"]

        # Instantiate the POSTed form with the POST data
        forms[form_type] = forms[form_type](
            data=request.POST, **forms_kwargs[form_type]
        )
    ...
    return render(
        request, "account/user/edit_account.html", {"target_user": user, **forms}
    )
